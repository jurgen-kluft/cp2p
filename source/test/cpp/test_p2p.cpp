...
namespace ncore
{
    namespace np2p
    {
        ...
        public:
            ...
            static MyAllocator* sCreate(alloc_t* inSystemAllocator)
            {
        ...
        public:
            ...
            static MyMessageAllocator* sCreate(alloc_t* inSystemAllocator)
            {
                // Create a heap, pool, caching or any other type of allocator
                return NULL;
            }
        ...
        public:
            ...
            void Start(u16 local_port, u16 remote_port, alloc_t* inSystemAllocator)
            {
                    ...
                    while (rcvd_messages->has_message())
                    {
                        peer* from = rcvd_messages->get_from();
                        {
                            ...
                            if (rcvd_messages->has_data())
                            {
                                ...
                                if (nflags::is_set(rcvd_messages->get_flags(), MSG_FLAG_ANNOUNCE))
                                {
                                ...
                                printf("info: message \"%s\"received from \"%s\"", (const char*)msgString, ip4_str);
                            }
                        }
...
UNITTEST_SUITE_BEGIN(p2p)
{
    UNITTEST_FIXTURE(main)
    {
        ...
        {
            ncore::np2p::MyPeer local;
            ncore::np2p::MyPeer tracker;
