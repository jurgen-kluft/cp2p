...
namespace ncore
{
	namespace np2p
	{
		...
		/// ---------------------------------------------------------------------------------------
		inline u8			read_u8 (byte const* ptr)				{ return (u8)*ptr; }
		inline s8			read_s8 (byte const* ptr)				{ return (s8)*ptr; }
		inline u16			read_u16(byte const* ptr)				{ u16 b0 = *ptr++; u16 b1 = *ptr++; return (u16)((b1<<8) | b0); }
		inline s16			read_s16(byte const* ptr)				{ u16 b0 = *ptr++; u16 b1 = *ptr++; return (s16)((b1<<8) | b0); }
		inline u32			read_u32(byte const* ptr)				{ u32 b0 = *ptr++; u32 b1 = *ptr++; u32 b2 = *ptr++; u32 b3 = *ptr++; return (u32)((b3<<24) | (b2<<16) | (b1<<8) | (b0)); }
		inline s32			read_s32(byte const* ptr)				{ u32 b0 = *ptr++; u32 b1 = *ptr++; u32 b2 = *ptr++; u32 b3 = *ptr++; return (s32)((b3<<24) | (b2<<16) | (b1<<8) | (b0)); }
		inline f32			read_f32(byte const* ptr)				{ u32 b0 = *ptr++; u32 b1 = *ptr++; u32 b2 = *ptr++; u32 b3 = *ptr++; u32 u = ((b3<<24) | (b2<<16) | (b1<<8) | (b0)); return *((f32*)&u); }
		inline u64			read_u64(byte const* ptr)				{ u64 l0 = read_u32(ptr); u64 l1 = read_u32(ptr+4); return (u64)((l1 << 32) | l0); }
		inline s64			read_s64(byte const* ptr)				{ u64 l0 = read_u32(ptr); u64 l1 = read_u32(ptr+4); return (s64)((l1 << 32) | l0); }
		inline f64			read_f64(byte const* ptr)				{ u64 l0 = read_u32(ptr); u64 l1 = read_u32(ptr+4); u64 ll = (u64)((l1 << 32) | l0); return *((f64*)ll); }

		inline void			write_u8 (byte * ptr, u8  b)			{ *ptr = b; }
		inline void			write_s8 (byte * ptr, s8  b)			{ *ptr = b; }
		inline void			write_u16(byte * ptr, u16 b)			{ for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_s16(byte * ptr, s16 b)			{ for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_u32(byte * ptr, u32 b)			{ for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_s32(byte * ptr, s32 b)			{ for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_f32(byte * ptr, f32 f)			{ u32 b = *((u32*)&f); for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_u64(byte * ptr, u64 b)			{ for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_s64(byte * ptr, s64 b)			{ for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		inline void			write_f64(byte * ptr, f64 f)			{ u64 b = *((u64*)&f); for (s32 i=0; i<sizeof(b); ++i) { ptr[i] = (u8)b; b = b >> 8; } }
		...
		byte*				message_block::get_data()
		{
			return data_;
		}

		const byte*		message_block::get_data() const
		{
			return data_;
		}
		...
		bool				message_reader::view_data(byte const*& data, u32 size)
		{
			...
			data = (byte const*)block_->get_data() + cursor_;
			cursor_ += size;
			return true;
		}
		...
		u32					message_writer::write(bool b)
		{
			...
			{
				write_u8((byte*)block_->get_data() + cursor_, b ? 1 : 0);
				cursor_ += 1;
				return 1;
			}
			return 0;
		...
		u32					message_writer::write(u8   b)
		{
			...
			{
				write_u8((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(u16  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_u16((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(s16  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_s16((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
			}
			return sizeof(b);
		}
		...
		u32					message_writer::write(u32  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_u32((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(s32  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_s32((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(u64  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_u64((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(s64  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_s64((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(f32  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_f32((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write(f64  b)
		{
			if (_can_write(block_, cursor_, sizeof(b)))
			{
				write_f64((byte*)block_->get_data() + cursor_, b);
				cursor_ += sizeof(b);
				return sizeof(b);
			}
			return 0;
		...
		u32					message_writer::write_data(const byte* _data, u32 _size)
		{
			if (_can_write(block_, cursor_, _size))
			{
				byte* dst = (byte*)block_->get_data() + cursor_;
		...
		{
			return nflags::is_set(flags_, MESSAGE_FLAG_EVENT);
		}
